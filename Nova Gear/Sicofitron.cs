...
{
    /// <summary>
    /// Description of Sicofitron.
    /// </summary>
    public partial class Sicofitron : Form
    {
        public Sicofitron()
        {
            //
        ...
        String archivo, ext, cad_con, tabla, cons_exc, hoja, sql, sql2, t_d, t_c, t_p_c, per, reg_pat, per2, credito, fecha, f1, f2, f3, f4, f5, ruta, id, cad_fech_des, cad_fech_has, nom_save, f1_1, nvo_stat_cap, ant_stat_cap, cons_fech_cop, inci, acierto_reg_cre, importe, retiro_p, rcv_o, rcv_p;
        ...
        public void filtro()
        {
            try
            {
                ...
                System.IO.File.Delete(@"capturator_sicofi/temp.txt");
                //Crear archivos nuevos
                //System.IO.File.Create(@"capturator/temp.txt");
                //Abrir archivo
                StreamWriter wr = new StreamWriter(@"capturator_sicofi/temp.txt");
                ...
                {    //CARGA COP
                    if (n == 1) //CARGAR BD
                    {
                        ...
                        consultamysql.Columns.Add("FOLIO SICOFI");
                ...
                for (i = 0; i < tot_rows; i++)
                {
                    ...
                    else
                    {
                        ...
                        fecha = "02";//comboBox3.SelectedItem.ToString(); //ajuste manual
						...
						*/	
                        ...
                        if ((credito.Length < 5) || (credito.Length > 5))
                        {
                            fecha_vac = 1;
                            x = x + 1;
                            error_list2[x] = i;
                        ...
                        if ((double.TryParse(importe, out importe_d))==true){	
                        }else{
                        		fecha_vac=1;
                        }
                        */
                        if (fecha_vac == 0)
                        {
                            if (tipo_carga == 1)
                            {
                                consultamysql.Rows.Add(reg_pat, credito);
                            }
                            else
                            {
                        ...
                        else
                        {
                            if (tipo_carga == 1)
                            {
                                tablarow2.Rows.Add(reg_pat, credito);
                            }
                            else
                            {
                    ...
                    if (fecha_vac == 0)
                    {
                        ...
                        {//Carga COP
                            ...
                            wr.WriteLine(fecha);//incidencia 02
                            wr.WriteLine(credito);//FOLIO SICOFI
                ...
                StreamWriter wr1 = new StreamWriter(@"capturator_sicofi/temp_aux.txt");
                wr1.WriteLine("0");
        ...
        public void finalizar()
        {
            ...
            try
            { 
            StreamReader rdr = new StreamReader(@"capturator_sicofi/errores_siscob.txt");
            StreamReader rdr1 = new StreamReader(@"capturator_sicofi/temp_aux.txt");
            ...
            if (n == 1)
            {
				...
				conex.guardar_evento("Se Capturaron en Siscob "+acierto_act+" Registros de Cambio a Incidencia 02 (SICOFI) de un total de "+tot_rows1);
            ...
            {  //finalizar cuando es archivo excel y access
                ...
                Invoke(new MethodInvoker(delegate
                {
                    ...
                    if (tipo_carga == 1)
                    {
                        do
                        {
                            dataGridView3.Rows[i].Cells[2].Value = "02";
                    ...
                    do
                    {
                        ...
                        if ((k < tot_errs) && (w == 0))
                        {
                            ...
                            if (candado == 0)
                            {
                                if (tipo_carga == 1)
                                {
                                    dataGridView3.Rows[i].Cells[3].Value = "CAPTURADO";
                                }
                                else
                                {
                    ...
                    while (i < tot_rows)
                    {
                        ...
                        {
                            consultamysql.Rows.Add(reg_pat, credito, "-", "0");
                        }
                        i++;
                    ...
                    if (tipo_carga == 1)
                    {
                        wr2.WriteLine("REGISTRO PATRONAL,FOLIO_SICOFI,INCIDENCIA,CAPTURA SISCOB");
                    }
                    else
                    {
                    ...
                    do
                    {
                        ...
                        {
                            fecha = dataGridView3.Rows[i].Cells[2].Value.ToString();
                            f1 = dataGridView3.Rows[i].Cells[3].Value.ToString();

                            wr2.WriteLine(reg_pat + "," + credito + "," + fecha + "," + f1);
                        }
					...
					conex.guardar_evento("Se Capturaron en Siscob "+acierto_act+" Registros de Cambio a Incidencia 02 (SICOFI) de un total de "+tot_rows);
                }));
            }
        ...
        void Sicofitron_Load(object sender, EventArgs e)
        {
            ...
            tablarow2.Columns.Add("FOLIO SICOFI");
            //tablarow2.Columns.Add("INCIDENCIA");
            ...
            if (guardar_resultados() == 1)
            {
                try
                {
                    ...
                    if (respuesta == DialogResult.Yes)
                    {
                        ...
                        //Borrar archivos para comenzar de 0
                        System.IO.File.Delete(@"capturator_sicofi/errores_siscob.txt");
                        System.IO.File.Delete(@"capturator_sicofi/acierto_siscob.txt");
                        System.IO.File.Delete(@"capturator_sicofi/aciertos_siscob.txt");
                        //Crear archivos nuevos
                        fichero = System.IO.File.Create(@"capturator_sicofi/errores_siscob.txt");

                        fichero1 = System.IO.File.Create(@"capturator_sicofi/acierto_siscob.txt");
                        fichero2 = System.IO.File.Create(@"capturator_sicofi/aciertos_siscob.txt");
                        ruta = fichero.Name;
                        fichero.Close();
                        fichero1.Close();
                        fichero2.Close();

                        StreamWriter wr1 = new StreamWriter(@"capture_sicofi.bat");
                        ...
                        if (tipo_carga == 1)
                        {
                            wr1.WriteLine("start astro_cap.exe");
                        }
                        else
                        {
                        ...
                        System.Diagnostics.Process.Start(@"capture_sicofi.bat");
                        conex.guardar_evento("Se Mando a capturar en Siscob "+consultamysql.Rows.Count.ToString()+" Registros de Cambio a Incidencia 02 (SICOFI).");
